...
def training(dataset, opt, pipe, iteration, saving_iterations, checkpoint_iterations, debug_from):
    ...
    background = torch.ones([dataset.feature_dim], dtype=torch.float32, device="cuda") \
        if dataset.white_background else \
        torch.zeros([dataset.feature_dim], dtype=torch.float32, device="cuda")
    ...
    if scale_aware_dim <= 0 or scale_aware_dim >= 32:
        ...
        fixed_scale_gate = torch.tensor(
            [[1 for j in range(32 - scale_aware_dim + i)] + [0 for k in range(scale_aware_dim - i)]
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            viewpoint_cam.feature_height = viewpoint_cam.image_height
            viewpoint_cam.feature_width = viewpoint_cam.image_width

            # N_mask
            mask_scales = viewpoint_cam.mask_scales.cuda()

            # skip cameras with no masks
            if len(mask_scales) == 0:
                continue

            # mask_scales and sam_masks can have different lengths, so align them
            n = min(len(mask_scales), sam_masks.shape[0])
            if n == 0:
                continue
            mask_scales = mask_scales[:n]
            sam_masks = sam_masks[:n]

            mask_scales, sort_indices = torch.sort(mask_scales, descending=True)
            sam_masks = sam_masks[sort_indices, :, :]

            # cap num_sampled_scales to available masks
            num_sampled_scales = min(8, len(mask_scales))
            sampled_scale_index = torch.randperm(len(mask_scales))[:num_sampled_scales]

            # use valid index for tmp[0] instead of -1 (which causes CUDA OOB)
            # tmp[0]'s scale value is immediately overwritten below, so the index only
            # matters for mask lookup — use last index (smallest scale) as safe placeholder.
            tmp = torch.zeros(num_sampled_scales + 2)
            tmp[1:len(sampled_scale_index) + 1] = sampled_scale_index
            tmp[-1] = len(mask_scales) - 1
            tmp[0] = len(mask_scales) - 1  # placeholder; overwritten by sampled_scales[0] = upper_bound + ...
            sampled_scale_index = tmp.long().cuda()

            sampled_scales = mask_scales[sampled_scale_index]

            # guard against no scales below upper_bound
            below_upper = mask_scales[mask_scales < upper_bound_scale]
            if len(below_upper) == 0:
                continue
            second_big_scale = below_upper.max()

            ray_sample_rate = opt.ray_sample_rate if opt.ray_sample_rate > 0 \
            ...
            # skip if no rays sampled (would cause empty tensor errors downstream)
            if sampled_ray.sum() == 0:
                continue
            ...
            # guard against constant weight (div by zero in normalization)
            weight_range = per_pixel_weight.max() - per_pixel_weight.min()
            if weight_range.item() == 0:
                per_pixel_weight = torch.ones_like(per_pixel_weight)
            else:
                per_pixel_weight = (per_pixel_weight - per_pixel_weight.min()) / weight_range * 9. + 1.
            ...
            # get plain Python ints for indexing
            for idx, si in enumerate(sampled_scale_index.cpu().tolist()):
                    ...
                    for j in range(si, -1, -1):
                        ...
                        if above.shape[0] > 0:
                            gt_vec[j, :] = torch.logical_and(
                                torch.logical_not(above.any(dim=0)), gt_vec[j, :]
                            )
                    gt_vec[si + 1:, :] = sam_masks_sampled_ray[si + 1:, :]
                else:
                    gt_vec = sam_masks_sampled_ray

                gt_corr = torch.einsum('nh,nj->hj', gt_vec.float(), gt_vec.float())
                gt_corr[gt_corr != 0] = 1
            ...
            # ensure sampled_scales stays 1D after squeeze
            if sampled_scales.dim() == 0:
                sampled_scales = sampled_scales.unsqueeze(0)

        render_pkg_feat = render_contrastive_feature(
            viewpoint_cam, feature_gaussians, pipe, background,
            norm_point_features=True, smooth_type='traditional',
        ...
        # N_sampled_scales x 32
        if scale_aware_dim <= 0 or scale_aware_dim >= 32:
            gates = scale_gate(sampled_scales.unsqueeze(-1))
        else:
            int_sampled_scales = ((1 - sampled_scales.squeeze()) * scale_aware_dim).long()
            # FIX 9: clamp to valid range [0, scale_aware_dim] — out-of-distribution scales
            # (e.g. the artificially inflated scale[0]) can produce values outside this range
            int_sampled_scales = torch.clamp(int_sampled_scales, 0, scale_aware_dim)
        ...
        # FIX 10: guard against zero-count denominators
        n_pos = consistent_positive.count_nonzero()
        n_neg = consistent_negative.count_nonzero()

        if n_pos == 0 or n_neg == 0:
            continue

        sampled_positive = torch.logical_and(consistent_positive, rand_num < sampled_num / n_pos)
        sampled_negative = torch.logical_and(consistent_negative, rand_num < sampled_num / n_neg)
        ...
        loss = (
            -(per_pixel_weight[:, sampled_mask_positive] * gt_corrs[:, sampled_mask_positive] * corr[:, sampled_mask_positive]).mean()
        ...
        if iteration % 10 == 0:
            progress_bar.set_postfix({
                "RFN": f"{rendered_feature_norm.item():.3f}",
                "Pos cos": f"{cosine_pos.item():.3f}",
                "Neg cos": f"{cosine_neg.item():.3f}",
                "Loss": f"{loss.item():.3f}",
            })
            progress_bar.update(10)

    scene.save_feature(
        iteration, target='contrastive_feature',
        smooth_weights=torch.softmax(smooth_weights, dim=-1) if smooth_weights is not None else None,
        smooth_type='traditional', smooth_K=opt.smooth_K
    )
    torch.save(
        scale_gate.state_dict(),
        os.path.join(scene.model_path, "point_cloud/iteration_{}/".format(iteration) + "scale_gate.pt")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target', default='contrastive_feature', const='contrastive_feature', nargs='?',
                        choices=['scene', 'seg', 'feature', 'coarse_seg_everything', 'contrastive_feature'])
